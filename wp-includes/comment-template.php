...
function get_post_reply_link($args = array(), $post = null) {
	$defaults = array(
		...
		'reply_text' => __('Deixe Um Comentário'),
		'login_text' => __('Loge-se para deixar um comentário'),
		'before'     => '',
		'after'      => '',
	);
...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Deixe Um Comentário' );
	if ( false === $replytext ) $replytext = __( 'Deixe Um Comentário %s' );
		...
		<<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $this->has_children ? 'parent' : '' ); ?>>
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<footer class="comment-meta">
					<div class="comment-author vcard">
						<?php if ( 0 != $args['avatar_size'] ) echo get_avatar( $comment, $args['avatar_size'] ); ?>
						<?php printf( __( '%s <span class="says">disse:</span>' ), sprintf( '<b class="fn">%s</b>', get_comment_author_link() ) ); ?>
					</div><!-- .comment-author -->
 ...
 *     @type string $label_submit         The translatable 'submit' button label. Default 'Deixe Um Comentário'.
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Nome' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . $html_req . ' /></p>',
		'email'  => '<p class="comment-form-email"><label for="email">' . __( 'E-mail' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Comentário', 'noun' ) . '</label> <textarea id="comment" name="comment" cols="45" rows="8" aria-describedby="form-allowed-tags" aria-required="true" required="required"></textarea></p>',
		/** This filter is documented in wp-includes/link-template.php */
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		/** This filter is documented in wp-includes/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logado como <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Deslogar?</a>' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes"><span id="email-notes">' . __( 'Seu e-mail não sera publicado.' ) . '</span>'. '</p>',
		...
		'title_reply_to'       => __( 'Deixe Um Comentário Para %s' ),
		'cancel_reply_link'    => __( 'Cancel reply' ),
		'label_submit'         => __( 'Enviar O Comentário' ),
